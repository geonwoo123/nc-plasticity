...
def build_promptmodel(modelname='vit_base_patch16_224', Prompt_Token_num=10, VPT_type="Deep", args=None):
    ...
    if modelname in ['vit_base_patch16_224', 'vit_base_patch16_clip_224']:
        model = VPT_ViT(Prompt_Token_num=Prompt_Token_num, VPT_type=VPT_type, args=args)
    elif modelname == 'vit_large_patch16_224':
        model = VPT_ViT(embed_dim=1024, depth=24, num_heads=16,
...
class VPT_ViT(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=200, embed_dim=768, depth=12,
        ...
        if VPT_type == "Deep":
            ...
            self.RSP = NDPrompt(args, embed_dim, 1, round(args["init_cls"] * self.args["prompt_pool_num"]), Prompt_Token_num / 2, key_dim=embed_dim)

        else:  # "Shallow"
            print("Using Shallow Prompt")
            self.TSP = DPrompt(embed_dim, num_classes / 2, Prompt_Token_num / 2)
            self.RSP = NDPrompt(embed_dim, 1, 20, Prompt_Token_num / 2)
...
class SimpleVitNet(BaseNet):
    def __init__(self, args, pretrained):
        ...
        self.feature_adapter = None  # injected by Learner; None = identity pass-through
